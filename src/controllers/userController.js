...
//module.export = createUserFunction()
...
userCtrl.getUserDetails = async (req, res) => {
    try{ let person = await User.find({username: req.body.usernmae})
         if(!person){
             res.status(400).send({message: 'user does not exist, check planet mars'})
         }else{
             res.status(200).send({message:'welcome to earth, the user exist', person})
         }
    }
    catch(error){
        console.log(error)
